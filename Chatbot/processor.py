...
model = load_model('Chatbot/chatbot_modelA.h5' , compile=False)
import json
import random
...
words = pickle.load(open('Chatbot/wordsA.pkl','rb'))
classes = pickle.load(open('Chatbot/classesA.pkl','rb'))
...
def getResponse(ints, intents_json, source_lang='en', target_lang='en', save_path="D:/PROJECT/AIRLINE/CODE/Deploy/Responses/response.mp3"):
    tag = ints[0]['intent']
    list_of_intents = intents_json['intents']
    translator = Translator()
    for i in list_of_intents:
